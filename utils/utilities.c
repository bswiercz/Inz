...
void write_p_val_to_file(struct state *state)
{
	 ...
	 */
	sprintf(&filename, "sts.%04ld.%ld.%ld.work", state->jobnum, state->tp.numOfBitStreams, state->tp.n);
	 ...
	 */
	sprintf(&filename, "sts.%04ld.%ld.%ld.pvalues", state->jobnum, state->tp.numOfBitStreams, state->tp.n);
